    ...
    with open('{}/{}/label.txt'.format(root, phase), 'r') as f:
        ...
        for line in tqdm(lines):
            ...
            if '#' in line:
                ...
                path = glob.glob(
                    '{}/{}/images/*/{}'.format(root, phase, os.path.basename(line)))[0]
                img = cv2.imread(path)
                height, width, _ = img.shape
                data[path] = list()
            else:
                box = np.array(line.split()[0:4],
                               dtype=np.float32)  # (x1,y1,w,h)
...
if __name__ == '__main__':
    ...
    for idx, data in enumerate(datas.keys()):
        ...
        folder_name = Path(data).parent.name
        img_name = Path(data).name
        folder_pict_txt.append(f'{folder_name}/{img_name}\n')
        pict_name = os.path.basename(data)
        out_img = 'widerface/val/images/{}'.format(pict_name)
        out_txt = 'widerface/val/labels/{}.txt'.format(
            os.path.splitext(pict_name)[0])
    ...
    with open('widerface/val/wider_val.txt', 'w') as f:
        f.writelines(folder_pict_txt)
