...
def detect_one(model, image_path, device, save_dir):
    # Load model
    img_size = 800
    conf_thres = 0.5
    iou_thres = 0.5
    ...
    save_name = Path(image_path).name
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str,
                        default='./weights/yolov5n-0.5.pt', help='model.pt path(s)')
    # file/folder, 0 for webcam
    parser.add_argument('--image', type=str,
                        default='data/images/test.jpg', help='source')
    parser.add_argument('--img-size', type=int, default=640,
                        help='inference size (pixels)')
    parser.add_argument('-s', '--save_dir', type=str,
                        default='result/detect', help='image save dir path')
    ...
    if Path(opt.image).is_file():
        detect_one(model, opt.image, device, opt.save_dir)
    if Path(opt.image).is_dir():
        for image_file in Path(opt.image).rglob('*'):
            detect_one(model, image_file.__str__(), device, opt.save_dir)
